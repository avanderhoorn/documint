import type { Editor, EditorHoverTarget } from "@/editor";
  ...
  const resolveHoverTarget = useEffectEvent(
    (event: PointerEvent<HTMLCanvasElement> | MouseEvent<HTMLCanvasElement>) => {
      ...
      return editor.resolveHoverTarget(
        currentState,
        getViewportRenderData(),
        resolvePointerPoint(event),
        commentState.liveRanges,
      );
    },
  );

  const applyHoverTarget = useEffectEvent(
    (target: EditorHoverTarget | null) => {
      ...
      if (target.kind === "task-toggle") {
        ...
        setHoverTarget((previous) =>
          previous?.kind === "task-toggle" &&
          previous.listItemId === target.listItemId
            ? previous
            : target,
        );
        return;
      ...
      if (target.commentThreadIndex !== null) {
        ...
        setHoverTarget((previous) =>
          previous?.kind !== "task-toggle" &&
          previous?.commentThreadIndex === threadIndex
            ? previous
            : target,
        );
        return;
