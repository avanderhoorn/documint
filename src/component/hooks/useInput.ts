...
export function useInput({
  ...
  const applyReplacementText = useEffectEvent(
    (state: typeof editorState, charsToDelete: number, replacement: string) => {
      ...
      // delegate to `replaceSelection`, which commits a single transaction
...
function applyKeyboardEvent(
  ...
  if (event.key === "ArrowUp" || event.key === "ArrowDown") {
    return editor.moveCaretVertically(state, viewport.layout, event.key === "ArrowUp" ? -1 : 1);
  }

  if (event.key === "PageUp" || event.key === "PageDown") {
    return editor.moveCaretByViewport(state, viewport.layout, event.key === "PageUp" ? -1 : 1);
  }
