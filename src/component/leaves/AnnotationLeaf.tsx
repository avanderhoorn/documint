...
export function AnnotationLeaf(props: AnnotationLeafProps) {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      rootRef.current
        ?.querySelector<HTMLTextAreaElement>(".documint-editor-comment-reply-field textarea")
        ?.focus();
    });
  ...
  const contentClassName = showCreateChrome
    ? `documint-editor-comment-popover documint-editor-comment-popover-create${isExpandedCreateMode ? " is-expanded" : ""}`
    : "documint-editor-comment-popover";
  ...
  return (
    <div className={contentClassName} ref={rootRef}>
      <div className={showCreateChrome ? "documint-editor-comment-popover-create-shell" : undefined}>
        {showCreateChrome ? (
          <LeafToolbar>
            <LeafToolbar.Button
              className="documint-editor-comment-popover-create-button"
              icon={MessageSquarePlus}
              label="Add comment"
              onClick={() => setIsExpanded(true)}
            />
            <LeafToolbar.Divider className="documint-editor-comment-popover-create-divider" />
            <LeafToolbar.Button
              active={activeMarks.includes("bold")}
              className="documint-editor-comment-popover-create-mark"
              icon={Bold}
              label="Bold"
              onClick={toggleBold}
            />
        ...
        <div className={showCreateChrome ? "documint-editor-comment-popover-create-content" : undefined}>
          {content}
        </div>
      </div>
    </div>
...
function AnnotationLeafBody({
  ...
  return (
    <>
      {showThreadChrome ? (
        <div className="documint-editor-comment-popover-header">
          <span className="documint-editor-comment-popover-age">{threadAge}</span>
          <div className="documint-editor-comment-popover-actions">
            <button
              className="documint-editor-leaf-action"
            ...
            <button
              className="documint-editor-leaf-action documint-editor-leaf-action-danger"
      ...
      {showThreadChrome && link ? (
        <div className="documint-editor-comment-popover-link">
          {link.title ? <div className="documint-editor-link-popover-title">{link.title}</div> : null}
          <div className="documint-editor-link-popover-url">{link.url}</div>
        </div>
      ) : null}
      <div className={`documint-editor-comment-thread${showRootComment ? "" : " is-empty"}`}>
        <article
          className={
            showRootComment
              ? `documint-editor-comment-message documint-editor-comment-message-root${isInitialCommentVisible ? " is-visible" : ""}`
              : "documint-editor-comment-message documint-editor-comment-message-root is-hidden"
          }
        >
          {rootComment ? (
            <p
        ...
        {comments.slice(1).map((comment, commentIndex) => {
          ...
          return (
            <article
              className="documint-editor-comment-message"
              key={`${comment.updatedAt}:${actualIndex}`}
            >
              {!isEditing ? (
                <div className="documint-editor-comment-message-meta">
              ...
              {isEditing ? (
                <EditInput
                  className="documint-editor-comment-input"
      ...
      <div
        className={`documint-editor-comment-reply${showThreadChrome ? "" : " is-standalone"}${isComposerVisible ? " is-visible" : ""}`}
      >
        <div className="documint-editor-comment-reply-field">
          <textarea
            className="documint-editor-comment-input"
            onChange={(event) =>
              mode === "create"
          ...
          />
          <button
            className="documint-editor-leaf-action documint-editor-comment-reply-submit"
