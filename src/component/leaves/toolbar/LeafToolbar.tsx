...
function LeafToolbarRoot({ children }: LeafToolbarProps) {
  return (
    <div className="documint-editor-leaf-toolbar">
...
function renderToolbarChild(child: ReactNode) {
  ...
  if (child.type === LeafToolbarButton) {
    ...
    return <LeafToolbarGroup><LeafToolbarIconButton {...props} /></LeafToolbarGroup>;
  }

  if (child.type === LeafToolbarDivider) {
    const { className } = child.props as LeafToolbarDividerProps;

    return <div className={resolveClassName("documint-editor-leaf-toolbar-divider", className)} />;
  }
  ...
  return (
    <button
      ...
      className={resolveClassName(
        "documint-editor-leaf-toolbar-button",
        active ? "active" : null,
        className,
      )}
      disabled={disabled}
...
function LeafToolbarMenuView({
  ...
  return (
    <LeafToolbarGroup>
      <div className="documint-editor-leaf-toolbar-menu-shell" ref={rootRef}>
        ...
        />
        {isOpen ? (
          <div className="documint-editor-leaf-toolbar-menu-popover" role="menu">
...
function LeafToolbarMenuButton({
  ...
  return (
    ...
    >
      ...
      <ChevronDown
        className={resolveClassName(
          "documint-editor-leaf-toolbar-menu-chevron",
          isOpen ? "is-open" : null,
        )}
        size={13}
        strokeWidth={2.2}
...
function renderToolbarMenuChild(
  child: ReactNode,
  onSelect: (value: string) => void,
) {
  if (!isValidElement(child)) {
  ...
  if (child.type === LeafToolbarMenuDivider) {
    ...
    return (
      <div
        className={resolveClassName("documint-editor-leaf-toolbar-menu-divider", className)}
        role="separator"
      />
    );
  }
  ...
  return (
    <button
      className="documint-editor-leaf-toolbar-menu-item"
      disabled={disabled}
      onPointerDown={(event) => {
        keepSelectionActive(event);
...
function LeafToolbarGroup({ children }: { children: ReactNode }) {
  return <div className="documint-editor-leaf-toolbar-group">{children}</div>;
}
