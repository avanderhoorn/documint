...
- **Render** (`render/`) - Owns immediate-mode canvas painting from prepared layout plus editor/runtime inputs such as selection, comments, presence, animations, and theme. Render does not build a retained scene graph after `ViewportLayout`; it translates layout/runtime data directly into canvas 2D drawing calls.
