...
import { getSelectionContext, type SelectionContext } from "./state";
import {
...
import {
  ...
  moveCaretToDocumentBoundary as moveCaretToDocumentBoundarySelection,
...
import { paintCanvasCaretOverlay, paintCanvasEditorSurface } from "./canvas/paint";
import type { EditorTheme } from "./canvas/theme";
import { hasRunningEditorAnimations } from "./canvas/animations";
import {
  insertText,
  insertLineBreak,
  deleteSelectionText,
...
export type { EditorSelectionPoint } from "./state";
...
export type Editor = {
  ...
  getSelectionContext(state: EditorState): SelectionContext;
  ...
  selectAll(state: EditorState): EditorStateChange;
...
export function createEditor(): Editor {
  ...
  const editor: Editor = {
    ...
    getSelectionContext(state) {
      return getSelectionContext(state.documentIndex, state.selection.anchor);
    },
    ...
    selectAll(state) {
      return createTransitionEditorStateChange(state, selectAll(state), false);
    },
    ...
    moveCaretVertically(state, layout, direction, extendSelection = false) {
      return createTransitionEditorStateChange(
        state,
        extendSelection
          ? extendSelectionVertically(state, layout, direction)
          : moveCaretVerticallySelection(state, layout, direction),
        false,
      );
    },
    moveCaretByViewport(state, layout, direction, extendSelection = false) {
      return createTransitionEditorStateChange(
        state,
        extendSelection
          ? extendSelectionByViewport(state, layout, direction)
    ...
    moveCaretToDocumentBoundary(state, boundary, extendSelection = false) {
      return createTransitionEditorStateChange(
        state,
        extendSelection
          ? extendSelectionToDocumentBoundary(state, boundary)
          : moveCaretToDocumentBoundarySelection(state, boundary),
        false,
      );
    },
