...
} from "./layout/hit-test";
import {
  createCanvasViewportLayout,
  type CanvasViewport,
...
function createEditorWithCache(renderCache: CanvasRenderCache): Editor {
  const editor: Editor = {
    ...
    prepareViewport(state, options, resources = emptyDocumentResources) {
      ...
      const viewportLayout = createCanvasViewportLayout(
        state.documentEditor,
        options,
        viewport,
        [
    ...
    resolveDragFocus(state, viewport, point, anchor, boundsTop) {
      return resolvePointerDragFocusPointAtLocation(viewport.layout, state, point, anchor, boundsTop);
    },
    ...
    paintSurface(state, viewport, context, options) {
      paintCanvasEditorSurface({
    ...
    paintCaret(state, viewport, context, options) {
      paintCanvasCaretOverlay({
        context,
        devicePixelRatio: options.devicePixelRatio,
        editorState: state,
...
function createContainerExtents(layout: DocumentLayout) {
