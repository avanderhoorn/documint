...
export type DocumentLayout = {
  ...
  regionMetrics: Map<string, { start: number; textLength: number }>;
  regionExtents: Map<string, { bottom: number; left: number; right: number; top: number }>;
...
export function createDocumentLayout(
  ...
  const regionExtents = new Map<string, { bottom: number; left: number; right: number; top: number }>();
  const regionMetrics = new Map(
    documentEditor.regions.map((container) => [
      container.id,
      {
  ...
  const blockDepth = new Map(documentEditor.blocks.map((block) => [block.id, block.depth]));
  ...
  for (let index = 0; index < documentEditor.regions.length; index += 1) {
    ...
    if (block?.type === "table") {
      ...
      const tableContainers = documentEditor.regions.slice(index, index + tableContainerIds.length);
  ...
  const blocks = documentEditor.blocks.map<DocumentLayoutBlock>((block) => {
  ...
  return {
    blocks,
    regionExtents: finalizeContainerExtents(lines, regionExtents),
...
function layoutSingleContainer(
  ...
  regionExtents: DocumentLayout["regionExtents"],
  ...
  updateContainerExtentFromLines(lines, container.id, regionExtents);

  return y;
}

function createContainerLineIndices(lines: DocumentLayoutLine[]) {
  const sortedLines = [...lines].sort((left, right) => left.top - right.top || left.left - right.left);
...
function finalizeContainerExtents(
  lines: DocumentLayoutLine[],
  initialExtents: Map<string, { bottom: number; left: number; right: number; top: number }>,
) {
  const extents = new Map(initialExtents);

  for (const line of lines) {
    const current = extents.get(line.regionId);
    const next = {
...
function updateContainerExtentFromLines(
  lines: DocumentLayoutLine[],
  regionId: string,
  extents: DocumentLayout["regionExtents"],
) {
...
export function findDocumentLayoutLineAtPoint(
  ...
  const containingContainer = [...layout.regionExtents.entries()].find(([, extent]) => {
    return (
      point.x >= extent.left &&
  ...
  return (
    candidates.find((candidate) => {
      const extent = layout.regionExtents.get(candidate.line.regionId);

      return extent ? point.x >= extent.left && point.x <= extent.right : false;
    }) ??
    [...candidates].sort((left, right) => {
      const leftExtent = layout.regionExtents.get(left.line.regionId);
      const rightExtent = layout.regionExtents.get(right.line.regionId);
...
export function findDocumentLayoutLineRange(
  layout: DocumentLayout,
  top: number,
  height: number,
) {
...
export function hitTestDocumentLayout(
  ...
  return {
    ...
    position: container.start + Math.min(container.textLength, line.start + offset),
    top: line.top,
  };
}
...
function resolveHorizontalDistance(
  x: number,
...
function resolveBoundaryOffset(boundaries: DocumentLineBoundary[], x: number) {
