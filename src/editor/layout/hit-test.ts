...
import type { EditorState, EditorSelectionPoint, EditorListItemMarker } from "../model";
...
export function resolveDragFocusPointAtLocation(
  ...
  return {
    regionId: anchor.regionId,
    offset: isSelectionPointBeforeAnchor(state, hit, anchor) ? 0 : anchorContainer.text.length,
  };
}
...
function isSelectionPointBeforeAnchor(
  state: EditorState,
  hit: { regionId: string; offset: number },
  anchor: EditorSelectionPoint,
) {
  const hitRegionIndex = state.documentIndex.regionOrderIndex.get(hit.regionId);
  const anchorRegionIndex = state.documentIndex.regionOrderIndex.get(anchor.regionId);

  if (hitRegionIndex === undefined || anchorRegionIndex === undefined) {
    return false;
  }

  if (hitRegionIndex !== anchorRegionIndex) {
    return hitRegionIndex < anchorRegionIndex;
  }

  return hit.offset < anchor.offset;
}
