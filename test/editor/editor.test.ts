...
test("removes hovered links while preserving linked text", () => {
  ...
  const transaction = editor.removeLink(state, region.id, linkRun.start, linkRun.end);

  expect(transaction).not.toBeNull();
  expect(serializeMarkdown(editor.getDocument(transaction!.state))).toBe("Paragraph with link.\n");
});
