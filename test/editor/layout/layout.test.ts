...
import { resolveCaretVisualLeft } from "@/editor/layout/hit-test";
...
test("hit-tests canvas layout coordinates back to semantic offsets", () => {
  const runtime = createDocumentEditor(
    parseMarkdown(`Paragraph with semantic offsets.\n`),
  );
  ...
  expect(hit?.position).toBe(paragraphContainer.start + 10);
});
...
test("advances the active caret across collapsed trailing spaces", () => {
  ...
  const paragraphContainer = state.documentEditor.regions[0];

  if (!paragraphContainer) {
    throw new Error("Expected paragraph container");
  }

  const beforeSpace = measureCaretTarget(layout, state.documentEditor, {
    regionId: paragraphContainer.id,
    offset: 5,
  });
  const afterSpace = measureCaretTarget(layout, state.documentEditor, {
    regionId: paragraphContainer.id,
    offset: 6,
  });
...
`),
  ...
  expect(layout.regionExtents.get(headerName.regionId)?.bottom).toBeGreaterThan(
...
`),
  ...
  const extent = layout.regionExtents.get(headerValue.regionId);

  if (!extent) {
    throw new Error("Expected table cell extent");
  }
...
`),
  ...
  const extent = layout.regionExtents.get(shortCellLine.regionId);

  if (!extent) {
    throw new Error("Expected short cell extent");
  }
...
test("keeps an empty layout line and caret target for inserted empty blocks", () => {
  ...
  const caret = measureCaretTarget(layout, state.documentEditor, state.selection.focus);
